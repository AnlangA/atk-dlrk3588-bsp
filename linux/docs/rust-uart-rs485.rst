...
This document lives in the out-of-tree ``atk-dlrk3588-bsp`` repository.
Paths below are relative to that repository; the kernel tree itself only
receives the patch series in ``linux/patches/``.

Out of tree (built with Kbuild ``M=`` against the patched kernel build):

* ``linux/drivers/rust_dw_uart.rs``: UART platform/controller driver.
* ``linux/drivers/rust_dw_uart/config.rs``: register encoding.
* ``linux/drivers/rust_chardev.rs``: character-device operations.
* ``linux/drivers/rust_chardev/ring.rs``: bounded FIFO indices.
* ``linux/dts/rk3588-atk-dlrk3588.dts``: board device tree, UART3 M2 wiring.
* ``app/rs485-test/``: Rust test application, peer, QEMU init and host tests.

In tree (``linux/patches/``, applied on the pinned mainline tag):
...
* ``rust/kernel/clk.rs``, ``rust/kernel/miscdevice.rs``, ``rust/kernel/fs/kiocb.rs``,
  ``rust/kernel/sync/barrier.rs``: small additions to existing abstractions.
* ``rust/helpers/serial.c``: inline/macro compatibility wrappers.
* ``rust/helpers/dmaengine.c``, ``rust/helpers/barrier.c``: DMAengine,
  workqueue and DMA barrier inline helpers.
...
The kernel needs ``CONFIG_RUST=y``, ``CONFIG_SERIAL_CORE=y``,
``CONFIG_DMA_ENGINE=y``, ``CONFIG_PL330_DMA=y`` and module unloading;
``CONFIG_DEBUG_FS=y`` exposes the DMA diagnostic counters. The fragments in
``linux/configs/`` add these to the arm64 ``defconfig``. Follow
``Documentation/rust/quick-start.rst`` for the Rust, Clang, libclang and
bindgen requirements; ``scripts/build-linux.sh`` runs
``make ARCH=arm64 LLVM=... rustavailable`` with the same settings first.

``make linux`` fetches nothing itself but expects ``make fetch`` to have
cloned the pinned tag; it applies ``linux/patches`` with ``git am``,
configures, builds ``Image`` and the in-tree modules, then compiles the two
out-of-tree modules and the device tree against that build::

    make -C external/linux O=build/linux ARCH=arm64 LLVM=1 \
        M=$PWD/linux/drivers MO=$PWD/build/linux-modules modules
    make -C external/linux O=build/linux ARCH=arm64 LLVM=1 \
        M=$PWD/linux/dts MO=$PWD/build/linux-dts

``modules_prepare`` is not a replacement for the full build. Deploy Image,
DTB and modules from the same build. A Rust module built here cannot be
inserted into an older running kernel which lacks Rust support or the
patched abstractions.

Build the application (``make app`` and ``make check``)::
...
linker when building on x86 (``scripts/build-app.sh`` derives it from
``AARCH64_CC``). ``Cargo.lock`` pins userspace dependencies. The
application's MSRV is independent of the kernel's toolchain minimum.
...
After deploying the new kernel, load and bind it explicitly::
    ...
    /usr/local/sbin/bind-uart3.sh --rust   # installed from board/bind-uart3.sh
...
After closing the UART application, restore the default controller::
    ...
    /usr/local/sbin/bind-uart3.sh --restore
...
``tests/kselftest.sh`` is the TAP entry point (formerly the kselftest
``run.sh``); it runs the ``rs485-test`` found next to it, in ``PATH`` or in
``RS485_TEST``. UART tests are opt-in through ``RS485_DEVICE=/dev/ttyRU0``
after starting the peer. Missing devices produce SKIP, not a false PASS.
``tests/board-smoke.sh`` chains the character-device test, binding and DMA
loopback at several baud rates on the board.
